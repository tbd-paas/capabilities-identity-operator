...
func NewIdentityCapabilitySubCommand(parentCommand *cobra.Command) {
	generateCmd := &cmdgenerate.GenerateSubCommand{
		...
		GenerateFunc: GenerateIdentityCapability, UseWorkloadManifest: true,
		WorkloadKind: "IdentityCapability",
	}

	generateCmd.Setup()
}

// GenerateIdentityCapability runs the logic to generate child resources for a
// IdentityCapability workload.
func GenerateIdentityCapability(g *cmdgenerate.GenerateSubCommand) error {
	...
	generateFuncMap := map[string]generateFunc{
		"v1alpha1": v1alpha1identitycapability.GenerateForCLI,
