...
IMG ?= "quay.io/tbd-paas/identity-operator:latest"
...
test-e2e: ## Run E2E tests against a running Kubernetes cluster.
	go test github.com/tbd-paas/capabilities-identity-operator/test/e2e -tags=e2e_test -count=1
...
docker-buildx: test ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name capabilities-identity-operator-builder
	$(CONTAINER_TOOL) buildx use capabilities-identity-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm capabilities-identity-operator-builder
	rm Dockerfile.cross
...
build-cli: ## Build the companion CLI.
	go build -o bin/identityctl cmd/identityctl/main.go
